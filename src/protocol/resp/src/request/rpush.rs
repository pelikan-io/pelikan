...
impl TryFrom<Message> for ListPushBack {
    type Error = Error;
...
impl From<&ListPushBack> for Message {
    fn from(value: &ListPushBack) -> Self {
        let mut vals = Vec::with_capacity(value.elements.len() + 2);

        vals.push(Message::BulkString(BulkString::new(b"RPUSH")));
...
mod tests {
    ...
    fn parser() {
        ...
        assert_eq!(
            parser
                .parse(b"rpush key m1 m2 m3 m4\r\n")
                .unwrap()
                .into_inner(),
            Request::ListPushBack(ListPushBack::new(b"key", &[b"m1", b"m2", b"m3", b"m4"]))
        );
        ...
        assert_eq!(
            parser
                ...
                .unwrap()
                .into_inner(),
            Request::ListPushBack(ListPushBack::new(b"key", &[b"m1", b"m2"]))
        );
    }
}
