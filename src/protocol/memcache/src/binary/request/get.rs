...
impl BinaryProtocol {
	pub(crate) fn parse_get_request<'a>(&self, input: &'a [u8], header: RequestHeader) -> IResult<&'a [u8], Get> {
	...
	fn parse_get_no_stats<'a>(&self, input: &'a [u8], header: RequestHeader) -> IResult<&'a [u8], Get> {
    ...
    pub(crate) fn compose_get_request(&self, request: &Get, buffer: &mut dyn BufMut) -> std::result::Result<usize, std::io::Error> {
    	self.compose_get_request_no_stats(request, buffer)
    }

    fn compose_get_request_no_stats(&self, request: &Get, buffer: &mut dyn BufMut) -> std::result::Result<usize, std::io::Error> {
    	if request.keys.len() != 1 {
			return Err(std::io::Error::new(std::io::ErrorKind::Other, "get request has multiple keys for binary protocol"));
		}
