...
impl BinaryProtocol {
    #[cfg(feature = "metrics")]
	pub(crate) fn parse_delete_request<'a>(&self, input: &'a [u8], header: RequestHeader) -> IResult<&'a [u8], Delete> {
        DELETE.increment();
	...
	fn _parse_delete_request<'a>(&self, input: &'a [u8], header: RequestHeader) -> IResult<&'a [u8], Delete> {
        // validation
    ...
    pub(crate) fn compose_delete_request(&self, request: &Delete, buffer: &mut dyn BufMut) -> std::result::Result<usize, std::io::Error> {
    	self._compose_delete_request(request, buffer)
    }

    fn _compose_delete_request(&self, request: &Delete, buffer: &mut dyn BufMut) -> std::result::Result<usize, std::io::Error> {
