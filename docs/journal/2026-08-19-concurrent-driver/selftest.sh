...
run() {
  ...
  grep -oE '(keys_with_lost_updates|keys_with_stale_token_success|phantom_values|false_misses|value_mismatches|replace_resurrected_deleted_key)[= ][^ ]*|destroyed = [0-9]+' "$out.run" | tr '\n' ' '
  echo
}
for mode in clean broken; do
  ...
  run flush $mode --threads 8 --after 0.5
done
